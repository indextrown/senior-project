...
token = "api"
channel = "#aws-ec2-alarm"
text = "x 크롤링 코드가 실행되었습니다. (30분마다 실행)"
text_end = "x 크롤링 코드가 종료되었습니다."
...
def search_x(search):
    ...
    while (ongoing):
        ...
        for title in titles:
            ...
            # 연속성 파악
            elements = [item[4] for item in old_titles]
            ...
            if url_gap not in elements:
                old_titles.append([nickname, user_id, clear_new_title, title_url, url_gap])
...
def crawling_main():
    ...
    try:
        ...
        if (check_login_selenium_x()):
            ...
            # old_titles가 빈 리스트인지 확인
            if not crawling_list:
                print("크롤링 결과가 없습니다.")
                return False # 크롤링 결과가 없을 때 False 반환
        
            # file i/o
            with open("output_crawling.txt", "w") as file:
            ...
            return True  # 성공적으로 완료되었을 때 True 반환
    except Exception as e:
        print(e)
        print("err")
        return False
...
#     # 코드 실행시 slack 알람 전송
#     alarm(token, channel, text)
    
#     # 코드 실행
#     main()

#     # 종료
#     alarm(token, channel, text_end)

#     g.gpt_api("output.txt")
