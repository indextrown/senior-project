...
def profile_crawling(profile_url_list):
    ...
    if check_login(driver, login_url):
        ...
        if not data_collected:
            print(f"{profile_name}에서 데이터를 찾지 못했습니다.")
            return False

        with open('output_crawling.txt', 'w', encoding='utf-8') as file:
            file.write('\n'.join([str(res) for res in all_titles]))

        with open('crawling_log.txt', 'a', encoding='utf-8') as file:
            file.write('\n'.join([str(res) for res in all_titles]))
...
def main():
    profile_url_list = [
                        "https://x.com/IVEstarship",
                        "https://x.com/NewJeans_ADOR",
                        "https://x.com/bts_bighit"
                        ]


    test = profile_crawling(profile_url_list)
